...
test("services/reports — scope, queries, aggregator", async (t) => {
  ...
  await t.test(
    "resolveProcessScope() — découvre les process via leur historique, filtre par serveur/process",
    async () => {
  ...
  await t.test(
    "resolveProcessScope() — respecte la visibilité de l'utilisateur (permission 'view')",
    async () => {
      const { resolveProcessScope } = require("../../lib/services/reports/scope");
  ...
  await t.test(
    "queries.js — alertsInPeriod/incidentsInPeriod/notificationsInPeriod/autoHealingInPeriod filtrent par plage et par process",
    async () => {
      ...
      const incidents = await queries.incidentsInPeriod({
        start: NOW - 2 * DAY,
        end: NOW,
        processNames: ["api"],
      });
  ...
  await t.test(
    "aggregator.generateReport() — compose un rapport complet à partir des sources existantes",
    async () => {
  ...
  await t.test(
    "aggregator.generateReport() — gros volume : beaucoup de process et d'échantillons, reste cohérent",
    async () => {
      const { generateReport } = require("../../lib/services/reports/aggregator");
      const manyProcesses = Array.from({ length: 30 }, (_, i) => `bulk-${i}`);
      for (const name of manyProcesses) {
        await seedProcessSamples(name, "local", {
          cpu: Math.random() * 100,
          memory: 1000 * (1 + Math.random()),
          count: 20,
        });
      }
  ...
  await t.test(
    "aggregator.generateReport() — Capacity Planning système à partir de l'historique persisté (system-history-store)",
    async () => {
      const { generateReport } = require("../../lib/services/reports/aggregator");
      ...
      assert.ok(
        ["already_exceeded", "high", "medium", "low"].includes(
          report.capacityPlanning.system.memory.confidence,
        ),
      );
    },
  );
