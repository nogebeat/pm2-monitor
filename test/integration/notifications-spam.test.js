...
test("Phase 5F — notification spam : CPU > seuil en continu ne déclenche pas une avalanche", async (t) => {
  ...
  await t.test("100 évaluations consécutives condition-vraie sur la même occurrence -> une seule notification", async () => {
    ...
    assert.equal(dispatchCount, 1, "une seule transition trigger->active sur 100 ticks à condition constante");
  ...
  await t.test("plusieurs occurrences distinctes visant le même provider dans la fenêtre de dédoublonnage -> une seule mise en file", async () => {
  ...
  await t.test("rate limiting : une avalanche de nombreuses occurrences distinctes sur le même provider est plafonnée", async () => {
    const { registry, calls } = makeCountingRegistry();
    const provider = await providerStore.create({ name: "P", type: "fake", configuration: {} });
    await routeStore.create({ name: "route", conditions: {}, providerIds: [provider.id] });
    ...
    for (let i = 0; i < 1000; i++) {
      const alert = { id: null, severity: "critical", metric: "cpu", targetType: "process", targetValue: `proc-${i}`, state: "active", triggeredAt: 1, lastSeenAt: 1 };
