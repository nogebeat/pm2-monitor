...
test("Phase 5F — Failure scenarios : chaque panne provider est absorbée, le pipeline reste fonctionnel", async (t) => {
  ...
  await t.test(
    "SMTP down (nodemailer rejette) : historique 'failed', pas d'exception propagée",
    async (t) => {
  ...
  await t.test(
    "Discord down (fetch rejette : DNS/connexion) : historique 'failed', pas d'exception propagée",
    async (t) => {
      withFetch(t, async () => {
  ...
  await t.test(
    "Telegram timeout (fetch qui n'aboutit jamais / AbortError) : historique 'failed', pas d'exception propagée",
    async (t) => {
      withFetch(t, async () => {
  ...
  await t.test(
    "Slack réponse invalide (200 mais corps inattendu) : historique 'failed' ou 'success' selon le contrat provider, jamais d'exception",
    async (t) => {
      withFetch(t, async () => ({
        ok: true,
        status: 200,
  ...
  await t.test(
    "Webhook générique indisponible (connexion refusée) : historique 'failed', pas d'exception propagée",
    async (t) => {
      withFetch(t, async () => {
        throw new Error("connect ECONNREFUSED");
      });
  ...
  await t.test(
    "Queue restart (jobs orphelins 'active' après un arrêt brutal) : repris par recoverStaleActiveJobs()",
    async () => {
      withGlobalFetchOk();
      ...
      const jobId = await dispatchQueue.queue.add({
        providerId: 1,
        alertId: null,
        notification: {},
        historyId: null,
  ...
  await t.test(
    "Database indisponible pendant l'écriture d'historique : enqueue() n'échoue jamais, juste dégradé",
    async () => {
      const registry = realRegistry();
      ...
      const provider = await providerStore.create({
        name: "P",
        type: "webhook",
        configuration: { url: "https://internal.example.invalid" },
        secrets: {},
      });
