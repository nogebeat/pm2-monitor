...
function run() {
  ...
  apiGet(
    ...
    .then((r) => {
      ...
      status.value = r.results.length ? (r.truncated ? t("fulltextModal.truncated", { total: r.total }) : "") : t("fulltextModal.noResult");
    })
    .catch((err) => {
      results.value = [];
...
<template>
  <ModalBase :title="t('fulltextModal.title')" hide-confirm @close="close">
    ...
    <label class="chk-inline" style="margin-top:8px;"><input v-model="regex" type="checkbox" /> {{ t("fulltextModal.regex") }}</label>
    ...
    <div class="search-results">
      ...
      <div v-for="row in results" :key="row.line" class="search-result-line">
        #{{ row.line }} · {{ new Date(row.t).toLocaleString(locale === "fr" ? "fr-FR" : "en-US") }} [{{ row.type }}/{{ row.level }}]<br />{{ row.text }}
      </div>
      <div v-if="status && results.length" class="hint-text">{{ status }}</div>
    </div>
  </ModalBase>
