...
const GLOBAL_ACTIONS = {
  ...
  // Backup & Restore (lib/services/backup/, lib/routes/backup.js) — Phase 19,
  // action globale, même raisonnement que servers_read/servers_manage : ni
  // l'export ni la restauration ne portent sur une app précise.
  // backup_export couvre la lecture du catalogue de sections et la
  // génération d'un fichier de backup (potentiellement avec des secrets
  // chiffrés, voir docs/backup-restore/README.md — c'est pourquoi cette
  // permission reste distincte de backup_restore plutôt que fusionnée).
  // backup_restore couvre la validation à blanc ET l'application réelle
  // d'un backup — une restauration pouvant recréer des comptes/permissions,
  // lib/routes/backup.js exige EN PLUS `req.user.isAdmin` sur la route de
  // restauration réelle (jamais sur la validation à blanc, en lecture
  // seule), à la manière de manage_users.
  backup_export: "Exporter la configuration (backup)",
  backup_restore: "Valider et restaurer un backup de configuration",

  // Reports & Capacity Planning (lib/services/reports/, lib/routes/reports.js)
  // — Phase 20, action globale, même raisonnement que servers_read/audit_read :
  // un rapport agrège des données déjà globales (alertes, incidents,
  // métriques process...), il n'est pas "sur" une app précise. Une seule
  // permission (lecture ET export) : un rapport n'expose jamais plus qu'une
  // consultation normale des sources qu'il agrège, chacune restant filtrée
  // par la visibilité habituelle de l'utilisateur (voir
  // lib/services/reports/scope.js) — contrairement à backup_export (Phase 19),
  // qui peut inclure des secrets chiffrés et justifie donc une permission à part.
  reports_read: "Voir et exporter les rapports (disponibilité, incidents, capacity planning...)",

  // Plugin System (lib/services/plugins/, lib/routes/plugins.js) — Phase 21,
  // action globale, même raisonnement que servers_read/servers_manage : un
  // plugin (code déposé dans plugins/, jamais "sur" une app précise) est un
  // objet de configuration du monitor lui-même. plugins_read couvre la
  // liste et le détail (statut, version, config) ; plugins_manage couvre
  // activer/désactiver et modifier la configuration d'un plugin — jamais
  // l'installation (aucune route ne permet d'ajouter du code, voir
  // docs/plugins/README.md#sécurité : un plugin s'ajoute uniquement en
  // déposant un dossier sur le serveur, hors de portée de l'API/permissions).
  plugins_read: "Voir la liste des plugins, leur statut, version et configuration",
  plugins_manage: "Activer/désactiver un plugin et modifier sa configuration",
};
...
const API_KEY_SCOPES = {
  ...
  "processes:restart":
    "Redémarrer un process (action sensible — seule action de mutation exposée à une clé API)",
