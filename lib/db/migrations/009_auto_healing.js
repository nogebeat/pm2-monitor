...
async function up(db) {
  if (db.driver === "mysql") {
    ...
    await db.run("CREATE INDEX IF NOT EXISTS idx_auto_healing_audit_process ON auto_healing_audit(process_name)");
    await db.run("CREATE INDEX IF NOT EXISTS idx_auto_healing_audit_created ON auto_healing_audit(created_at)");
  }
...
module.exports = {
  ...
  description: "Tables auto_healing_settings / auto_healing_state / auto_healing_audit (Phase 7 — Auto-Healing).",
  up,
  down,
};
