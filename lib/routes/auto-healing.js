...
function createAutoHealingRouter(service) {
  ...
  router.put("/settings", auth.requirePermission("authealing_manage"), async (req, res) => {
    try {
      ...
      recordEvent({
        user: req.user,
        action: ACTIONS.AUTO_HEALING_ACTION,
        target: "settings",
        targetType: "auto_healing",
        status: "success",
        ip: req.ip,
        metadata: { op: "settings_update", enabled: updated.enabled, maxAttempts: updated.maxAttempts },
      });
      res.json(updated);
    } catch (e) {
      recordEvent({
        ...
        status: "failed",
        ip: req.ip,
        metadata: { op: "settings_update", error: e.message },
      });
  ...
  router.post("/state/:process/unblock", auth.requirePermission("authealing_manage"), async (req, res) => {
    try {
      ...
      recordEvent({
        ...
        status: "failed",
        ip: req.ip,
        metadata: { op: "unblock", error: e.message },
      });
      res.status(500).json({ error: e.message });
    }
  });
