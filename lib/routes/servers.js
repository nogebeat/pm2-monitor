 ...
 *
 * `processHistory` (lib/services/process-history/) est injecté pour les
 * routes /:key/processes/:processName/{metrics,analytics} (Phase 11) — même
 * instance que celle utilisée par lib/routes/processes.js pour le serveur
 * local, pas de second service.
 */
...
function createServersRouter({ agentHub, processHistory }) {
  ...
  router.get("/", auth.requirePermission("servers_read"), async (req, res) => {
    try {
      ...
      // requireServerAccess (elle liste, ne cible pas un :key précis), donc
      // le filtrage par resourceScopes.servers doit être géré explicitement
      // ici — même raisonnement que lib/routes/processes.js#GET /processes.
      if (!req.user && req.apiKeyAuth) {
        const allowedServers = req.apiKeyAuth.resourceScopes && req.apiKeyAuth.resourceScopes.servers;
        const visible =
          Array.isArray(allowedServers) && allowedServers.length
            ? all.filter((s) => allowedServers.includes(s.serverKey))
            : all;
  ...
  router.post("/:key/action", auth.requireServerAccess(serverKeyParam), async (req, res) => {
    try {
      const server = await store.getByKey(req.params.key);
      if (!server) return res.status(404).json({ error: "Serveur introuvable." });
  ...
  // Metrics/Analytics (lib/services/process-history/, Phase 11) pour un
  // process d'un serveur distant. Distinct de /api/processes/:id/metrics
  // (lib/routes/processes.js), qui résout `:id` via pm2.describe() — donc
  // strictement local, ne peut pas voir un process d'agent. Avant la
  // migration 014_process_metrics_server_key.js, ces deux routes
  // n'existaient pas du tout : aucune donnée Metrics/Analytics n'était
  // consultable pour un serveur distant, même si `record()` avait été
  // branché côté collecte.
  router.get(
    "/:key/processes/:processName/metrics",
    auth.requireServerAccess(serverKeyParam),
    async (req, res) => {
      const { processName } = req.params;
      if (!permissions.hasPermission(req.user, processName, "view")) {
        return res.status(403).json({ error: "Action non autorisée pour cette app." });
      }
      try {
        const { start, end, resolution, metrics } = req.query;
  ...
  router.get(
    "/:key/processes/:processName/analytics",
    auth.requireServerAccess(serverKeyParam),
    async (req, res) => {
      const { processName } = req.params;
      if (!permissions.hasPermission(req.user, processName, "view")) {
