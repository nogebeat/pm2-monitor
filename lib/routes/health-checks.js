...
function createHealthChecksRouter() {
  ...
  router.post("/", auth.requirePermission("health_checks_create"), async (req, res) => {
    try {
      ...
      recordEvent({
        user: req.user,
        action: ACTIONS.HEALTH_CHECK_CHANGE,
        target: check.name || String(check.id),
        targetType: "health_check",
        status: "success",
        ip: req.ip,
        metadata: { op: "create", checkId: check.id, type: check.type },
      });
  ...
  async function updateCheck(req, res) {
    try {
      ...
      recordEvent({
        ...
        status: "success",
        ip: req.ip,
        metadata: { op: "update", checkId: updated.id, fields: Object.keys(req.body || {}) },
      });
      res.json(updated);
    } catch (e) {
      res.status(400).json({ error: e.message });
  ...
  router.post("/:id/enable", auth.requirePermission("health_checks_update"), async (req, res) => {
    try {
      ...
      recordEvent({
        ...
        status: "success",
        ip: req.ip,
        metadata: { op: "enable", checkId: updated.id },
      });
      res.json(updated);
    } catch (e) {
      res.status(500).json({ error: e.message });
  ...
  router.post("/:id/disable", auth.requirePermission("health_checks_update"), async (req, res) => {
    try {
      ...
      recordEvent({
        ...
        status: "success",
        ip: req.ip,
        metadata: { op: "disable", checkId: updated.id },
      });
      res.json(updated);
    } catch (e) {
      res.status(500).json({ error: e.message });
  ...
  router.delete("/:id", auth.requirePermission("health_checks_delete"), async (req, res) => {
    try {
      ...
      recordEvent({
        ...
        status: "success",
        ip: req.ip,
        metadata: { op: "delete", checkId: Number(req.params.id) },
      });
      res.json({ ok: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
