...
function createAuditRouter() {
  ...
  router.get("/", auth.requirePermission("audit_read"), async (req, res) => {
    try {
      ...
      if (status && !STATUSES.includes(status)) {
        return res
          .status(400)
          .json({ error: `status invalide: "${status}". Attendu: ${STATUSES.join(", ")}.` });
      }
  ...
  // IMPORTANT : doit être déclarée AVANT "/:id", sinon Express matche
  // "/catalog" comme "/:id" avec id="catalog" (Number("catalog") = NaN,
  // getById renvoie null → 404) et cette route ne répond jamais.
