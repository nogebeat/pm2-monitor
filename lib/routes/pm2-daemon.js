 ...
 */
function createPm2DaemonRouter({ logStore } = {}) {
  const router = express.Router();

  router.post(
    "/save",
    auth.requirePermission("pm2_save", null, { action: ACTIONS.PM2_SAVE, targetType: "pm2_daemon" }),
    (req, res) => {
  ...
  router.post(
    "/resurrect",
    auth.requirePermission("pm2_resurrect", null, {
  ...
  router.post("/flush-all", auth.requirePermission("pm2_flush_all"), (req, res) => {
    // Même raisonnement que /processes/:id/flush (lib/routes/processes.js) :
    // pm2.flush() ne vide que les fichiers natifs PM2, jamais le LogStore
    // persistant — on vide donc aussi celui-ci pour rester cohérent avec ce
    // que "Vider TOUS les logs de TOUS les process" laisse entendre côté UI.
    const action = pm2Actions.flush(pm2).then(() => {
      if (logStore) logStore.clearAll();
    });
  ...
  router.post(
    "/kill",
    auth.requirePermission("pm2_kill", null, { action: ACTIONS.PM2_KILL, targetType: "pm2_daemon" }),
    (req, res) => {
