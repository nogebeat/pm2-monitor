...
const reportsSystemHistory = require("./services/reports/system-history-store");
...
function startPolling({
  ...
  setInterval(() => {
    ...
    // Reports & Capacity Planning (Phase 20) : persiste (au plus une fois
    // toutes les 5 min, voir system-history-store.js#PERSIST_INTERVAL_MS)
    // le même échantillon système déjà calculé ci-dessus — pas de seconde
    // collecte, juste une rétention plus longue que les 24h en mémoire de
    // historyStore pour permettre des projections weekly/monthly.
    reportsSystemHistory
      .recordFromHistoryStore(historyStore.samples[historyStore.samples.length - 1])
      .catch((e) => {
        console.error("Erreur de persistance de l'historique système (reports) :", e.message);
      });
    io.emit("system", snap);
