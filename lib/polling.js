 ...
 */
function startPolling({ io, historyStore, processHistory, dispatchAlertTransition, alertsEnabled, alertsEvalIntervalMs }) {
  ...
  if (alertsEnabled || processHistory.config.enabled) {
    setInterval(() => {
      pm2.list((err, list) => {
