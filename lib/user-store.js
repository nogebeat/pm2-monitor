...
const db = require("./db");
const serverScope = require("./services/servers/user-scope");
...
function rowToUser(row) {
  ...
  return {
    ...
    // Phase 18 — étiquette informative seulement (voir migration
    // 020_rbac_api_keys.js et applyRole() ci-dessous) : jamais lue par
    // lib/permissions.js#hasPermission, qui continue à ne dépendre que de
    // is_admin + des lignes `permissions`.
    role: row.role || null,
...
/** Retourne l'utilisateur (avec permissions + scoping serveurs chargés) ou null. */
async function getUserWithPermissions(id) {
  const row = await db.get("SELECT * FROM users WHERE id = ?", [id]);
  if (!row) return null;
  const user = rowToUser(row);
  user.permissions = await loadPermissions(user.id);
  // Phase 10 — Multi-server : liste vide = pas de restriction (voir
  // lib/permissions.js#hasServerAccess et lib/services/servers/user-scope.js).
  user.allowedServerKeys = await serverScope.listAllowedServerKeys(user.id);
  return user;
}
...
async function listUsers() {
  ...
  for (const u of users) {
    ...
    u.allowedServerKeys = await serverScope.listAllowedServerKeys(u.id);
  }
  return users;
}
...
/** Remplace le scoping serveurs d'un utilisateur (Phase 10). Liste vide = aucune restriction. */
async function replaceAllowedServers(userId, serverKeys) {
  await serverScope.replaceAllowedServers(userId, serverKeys);
}

/**
 * Applique un rôle prédéfini (Phase 18, lib/permissions.js#ROLES) à un
 * utilisateur : remplace intégralement ses permissions par celles du rôle
 * (comme le ferait un admin à la main via replacePermissions), pose
 * `is_admin` en conséquence (le rôle "admin" court-circuite la table
 * `permissions`, voir hasPermission()), et note l'étiquette `role` posée
 * (purement informative — voir rowToUser()). N'introduit AUCUN second
 * système d'autorisation : après cet appel, hasPermission() se comporte
 * exactement comme si les mêmes lignes avaient été écrites une par une.
 */
async function applyRole(userId, roleName) {
  const role = ROLES[roleName];
  if (!role) {
    throw new Error(`Rôle invalide : "${roleName}". Valeurs acceptées : ${Object.keys(ROLES).join(", ")}.`);
  }
  await setAdmin(userId, role.isAdmin);
  await replacePermissions(userId, role.isAdmin ? [] : role.permissions);
