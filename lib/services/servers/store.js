...
function rowToServer(row) {
  ...
  if (row.last_processes) {
    try {
      lastProcesses = JSON.parse(row.last_processes);
    } catch (e) {
      lastProcesses = null;
    }
  }
  return {
    ...
    // Dernier snapshot de process reçu de l'agent (Phase 15 — Prometheus,
    // migration 017) : persisté au même rythme que `snapshot` ci-dessus
    // (voir touchStatus()), survit à un redémarrage du serveur central,
    // contrairement à un simple cache mémoire. NULL pour le serveur local
    // (process lus en direct via pm2.list(), voir lib/routes/metrics.js) et
    // pour tout serveur distant sans heartbeat encore reçu.
...
function validateEnvironment(environment) {
  ...
  if (!ENVIRONMENTS.includes(env)) {
    throw new Error(
      `Environnement invalide : "${environment}". Valeurs acceptées : ${ENVIRONMENTS.join(", ")}.`,
    );
  }
  return env;
}
...
async function touchStatus(serverKey, { status, agentVersion, protocolVersion, snapshot, processes } = {}) {
  ...
  // Phase 15 — Prometheus : persiste le dernier snapshot process reçu, au
  // même rythme que `snapshot` ci-dessus (register + heartbeats, voir
  // lib/realtime/agent-hub.js), pour que GET /metrics puisse le lire même
  // après un redémarrage du serveur central (voir migration 017).
  if (processes !== undefined) {
    fields.push("last_processes = ?");
    params.push(Array.isArray(processes) && processes.length ? JSON.stringify(processes) : null);
  }
  params.push(serverKey);
  await db.run(`UPDATE servers SET ${fields.join(", ")} WHERE server_key = ?`, params);
}
