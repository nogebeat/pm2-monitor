...
class AnomalyDetectionService {
  ...
  async _evaluateReading(rule, target, reading) {
    ...
    let alert = await this.alertEngine.evaluate(
      syntheticRule,
      target,
      Number(detection.absZScore.toFixed(2)),
    );
    ...
    if (detection.anomalous) {
      ...
      if (reading.usedFallback) {
        explanation +=
          " (baseline calculée à partir d'un historique en mémoire : process-history insuffisant.)";
      }
