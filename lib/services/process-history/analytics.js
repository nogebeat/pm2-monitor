 ...
 */
async function computePeriodStats({ processName, start, end, resolution }) {
  if (!(end > start)) return emptyPeriodStats(start, end);

  let agg;
  if (resolution === "raw") {
    const rows = await store.queryRaw({ processName, start, end });
    agg = aggregateSamples(rows);
  } else {
    const rows = await store.queryRollup({ processName, resolution, start, end });
    agg = aggregateRollupBuckets(rows);
  }

  // Crashes : réutilise le catalogue d'événements existant (pagination
  // obligatoire côté event-store, mais on ne veut que le total -> limit=1).
 ...
 */
async function computeAnalytics({ processName, start, end, resolution, compare = true }) {
  const current = await computePeriodStats({ processName, start, end, resolution });
