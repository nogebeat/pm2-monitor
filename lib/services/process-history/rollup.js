...
async function rollupRawToMedium(processName, bucketMs, now) {
  ...
  for (const bucketStart of buckets) {
    const samples = await store.queryRaw({ processName, start: bucketStart, end: bucketStart + bucketMs - 1 });
...
async function runMaintenance(config, now = Date.now()) {
  const report = { mediumBucketsWritten: 0, longBucketsWritten: 0, rawPurged: 0, mediumPurged: 0, longPurged: 0 };
