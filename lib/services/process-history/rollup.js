...
async function rollupMediumToLong(processName, serverKey, bucketMs, now) {
  ...
  for (const bucketStart of buckets) {
    ...
    const points = rows.map((r) => ({
      ...
      heapTotal: { avg: r.heap_total_avg, min: r.heap_total_min, max: r.heap_total_max, p95: r.heap_total_p95 },
      eventLoopLag: {
...
async function runMaintenance(config, now = Date.now()) {
  ...
  for (const { processName, serverKey } of rawKeys) {
    report.mediumBucketsWritten += await rollupRawToMedium(processName, serverKey, config.mediumBucketMs, now);
  }
