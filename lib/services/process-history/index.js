...
class ProcessHistoryService {
   ...
   * @param {number} now
   * @param {string} [serverKey] - "local" (défaut, hub lui-même) ou le
   *   server_key d'un agent distant (lib/realtime/agent-hub.js, à la
   *   réception d'un heartbeat — voir migration 014_process_metrics_server_key.js)
   */
  async record(processList, now = Date.now(), serverKey = store.DEFAULT_SERVER_KEY) {
   ...
   * @param {string} [opts.serverKey] - "local" (défaut) ou le server_key d'un serveur distant
   ...
   */
  async query({ processName, serverKey = store.DEFAULT_SERVER_KEY, start, end, resolution, metrics } = {}) {
    ...
    if (effectiveResolution === "raw") {
      const rows = await store.queryRaw({ processName, serverKey, start: rangeStart, end: rangeEnd });
   ...
   */
  async analytics({ processName, serverKey = store.DEFAULT_SERVER_KEY, start, end, resolution, compare = true } = {}) {
    if (!processName) throw new Error("processName requis.");
    if (resolution && !RESOLUTIONS.includes(resolution)) {
      throw new Error(`resolution invalide: "${resolution}". Attendu: ${RESOLUTIONS.join(", ")}.`);
    }
    ...
    return { processName, serverKey, resolution: effectiveResolution, start: rangeStart, end: rangeEnd, ...result };
  }
}
