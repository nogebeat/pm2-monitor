...
function classifySmtpError(err) {
  ...
  if (code === "ETIMEDOUT")
    return { errorCode: "TIMEOUT", safeMessage: "La connexion au serveur SMTP a dépassé le délai imparti." };
  if (code === "EAUTH")
    return {
      errorCode: "AUTH_ERROR",
      safeMessage: "Authentification SMTP refusée (identifiants invalides ?).",
    };
  ...
  if (code === "EENVELOPE")
    return { errorCode: "HTTP_ERROR", safeMessage: "Expéditeur ou destinataire refusé par le serveur SMTP." };
  ...
  async send(notification, config) {
    ...
    if (!to) {
      return failureResult(this.type, {
        ...
        safeMessage: "Aucun destinataire (to) fourni.",
      });
    }
    ...
    const from = config.fromName
      ? `"${String(config.fromName).trim()}" <${String(config.fromEmail).trim()}>`
    ...
    try {
      ...
      return successResult(this.type, {
