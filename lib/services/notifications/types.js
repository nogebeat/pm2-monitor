...
class NotificationProvider {
  ...
  constructor(type, label) {
    ...
    /**
     * Phase 5C : liste des clés de configuration considérées comme des
     * secrets pour ce provider (ex. ["webhookUrl"], ["botToken"],
     * ["username", "password"]). Un provider sans secret (aucun cas actuel)
     * laisse le tableau vide. Utilisé par lib/routes/notifications.js pour
     * scinder un formulaire admin (champs fusionnés) en `configuration`
     * (public) / `secrets` (chiffré) sans dupliquer cette connaissance
     * ailleurs — voir provider-store.js.
     */
    this.secretFields = [];
  }
