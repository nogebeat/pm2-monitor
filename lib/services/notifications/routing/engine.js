 ...
 *
 * Silencing (Phase 14 — Incident Management & Alert Silencing) : un silence
 * actif (lib/services/incidents/silence-store.js) ne supprime jamais
 * l'alerte ni l'événement qui l'a déclenchée — il empêche seulement la
 * notification correspondante d'être envoyée (voir dispatch() ci-dessous).
 * Vérifié APRÈS le matching des routes mais AVANT tout envoi (direct ou via
 * dispatchQueue) : une entrée `notification_history` de statut "silenced"
 * est tout de même écrite par route/provider matché, pour que l'opérateur
 * voie que la notification a été supprimée plutôt que silencieusement
 * absente. Optionnel (`deps.silenceStore`) : sans lui, aucune vérification
 * n'est faite (comportement historique, conservé pour les tests unitaires
 * existants de ce module qui ne connaissent pas encore les silences).
 */
...
/**
 * Comme matchesList, mais pour un filtre à comparer à un ENSEMBLE de valeurs
 * (ex: les tags ou les groupes d'un process, potentiellement plusieurs) :
 * matche si l'intersection est non vide. Utilisé par routeMatches() pour
 * `conditions.tag`/`conditions.group` (voir _resolveProcessOrg ci-dessous).
 */
function matchesAny(list, values) {
  if (!Array.isArray(list) || list.length === 0) return true; // pas de filtre = tout passe
  if (!Array.isArray(values) || values.length === 0) return false;
  return list.some((v) => values.includes(String(v)));
}
...
class RoutingEngine {
   ...
   *   processOrgStore?: import("../../process-organization/store"),
   *   silenceStore?: import("../../incidents/silence-store"),
   * }} deps
   */
  constructor({
    routeStore,
    ...
    // Optionnel (Phase 13) : résout tags/environnement/groupes d'un process
    // pour évaluer conditions.tag/environment/group (voir routeMatches() et
    // _resolveProcessOrg() ci-dessous). Sans lui, ces filtres ne matchent
    // jamais (comportement historique Phase 5D, conservé par
    // _resolveProcessOrg() qui renvoie null si processOrgStore est absent).
    this.processOrgStore = processOrgStore || null;
    // Optionnel (Phase 14) : voir commentaire de classe ci-dessus (Silencing).
    this.silenceStore = silenceStore || null;
  }

  // --- Matching --------------------------------------------------------

  /**
   * `conditions.server` est accepté pour compatibilité future (déploiement
   * multi-hôte) et interprété prudemment : seules les cibles "system"
   * peuvent matcher un filtre `server` non vide (une cible "process" n'a pas
   * de serveur au sens de ce filtre — l'Alert Engine reste mono-hôte, voir
   * lib/services/alerts/engine.js).
   *
   * `conditions.tag`/`conditions.environment`/`conditions.group` (Phase 13 —
   * Tags, Environments & Process Groups) sont résolus via `processOrg`,
   * l'organisation du process ciblé par l'alerte (voir _resolveProcessOrg(),
   * lib/services/process-organization/store.js#getOrganizationForProcess).
   * `processOrg` vaut `null` pour une alerte "system" (pas de process, donc
   * pas de tag/environnement/groupe) ou si aucun `processOrgStore` n'a été
   * injecté au constructeur — dans ce cas ces trois filtres, s'ils sont
   * définis sur la route, ne matchent jamais (repli sûr, même principe que
   * l'ancien comportement documenté ici avant la Phase 13).
   */
  routeMatches(route, alert, processOrg) {
    ...
    if (Array.isArray(c.tag) && c.tag.length > 0) {
      if (!matchesAny(c.tag, processOrg ? processOrg.tags : [])) return false;
    }
    if (Array.isArray(c.environment) && c.environment.length > 0) {
      if (!matchesList(c.environment, processOrg ? processOrg.environment : null)) return false;
    }
    if (Array.isArray(c.group) && c.group.length > 0) {
      if (!matchesAny(c.group, processOrg ? processOrg.groups : [])) return false;
    }
    return true;
  }

  /**
   * Résout l'organisation (tags/environnement/groupes) du process ciblé par
   * l'alerte, une seule fois par dispatch() (partagée par toutes les
   * routes) — voir routeMatches(). Retourne `null` pour une alerte "system"
   * ou si `processOrgStore` n'a pas été injecté.
   */
  async _resolveProcessOrg(alert) {
    if (!alert || alert.targetType !== "process" || !alert.targetValue) return null;
    if (!this.processOrgStore) return null;
    try {
      return await this.processOrgStore.getOrganizationForProcess(alert.targetValue);
    } catch (e) {
      // Une organisation illisible ne doit jamais empêcher le dispatch des
      // routes qui ne filtrent pas sur tag/environment/group (voir dispatch()
      // : ce module ne doit jamais lancer vers la boucle de monitoring).
      console.error("Erreur de résolution tags/environnement/groupes (routing) :", e.message);
      return null;
    }
  }

  async findMatchingRoutes(alert, processOrg) {
    const routes = await this.routeStore.list({ enabledOnly: true });
    return routes.filter((route) => this.routeMatches(route, alert, processOrg));
  }

  /**
   * Phase 14 — vrai si un silence actif couvre cette alerte (voir
   * lib/services/incidents/silence-store.js#isSilenced). `processOrg` est
   * réutilisé tel quel (déjà résolu une fois par dispatch(), voir
   * ci-dessous) pour éviter une seconde requête sur l'organisation du
   * process. `false` si `silenceStore` n'a pas été injecté (comportement
   * historique).
   */
  async _isSilenced(alert, processOrg) {
    if (!this.silenceStore) return false;
    try {
      return await this.silenceStore.isSilenced(alert, processOrg);
    } catch (e) {
      // Un silence store illisible ne doit jamais empêcher l'envoi des
      // notifications par ailleurs légitimes — on considère "non silencé"
      // et on journalise, même logique de repli sûr que _resolveProcessOrg.
      console.error("Erreur de vérification de silence (routing) :", e.message);
      return false;
    }
  }
  ...
  async dispatch(alert, event) {
    if (!alert) return [];
    try {
      const processOrg = await this._resolveProcessOrg(alert);
      const routes = await this.findMatchingRoutes(alert, processOrg);
      const silenced = await this._isSilenced(alert, processOrg);
      const results = [];
      for (const route of routes) {
        ...
        for (const providerId of providerIds) {
          if (silenced) {
            results.push(await this._recordSilenced(providerId, alert));
            continue;
          }
  ...
  /** Route matchée mais alerte silencée : pas d'envoi, juste une trace dans notification_history. */
  async _recordSilenced(providerId, alert) {
...
module.exports = { RoutingEngine, matchesList, matchesAny };
