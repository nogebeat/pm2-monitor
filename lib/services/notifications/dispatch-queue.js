...
class NotificationDispatchQueue {
  ...
  _isRateLimited(providerId) {
    const now = this.now();
    const bucket = (this._rateBuckets.get(providerId) || []).filter((ts) => now - ts < this.rateLimit.windowMs);
  ...
  async handleJob(payload, job) {
    ...
    const responseTimeMs = result && result.responseTime != null ? result.responseTime : this.now() - startedAt;
