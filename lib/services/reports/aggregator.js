...
async function buildCapacityPlanning({ start, end }) {
  ...
  const project = (key, threshold) => ({
    ...
    dataWindowMs,
    note:
      "Projection système basée sur l'historique persisté (échantillonné toutes les 5 minutes, voir lib/services/reports/system-history-store.js) — une tendance statistique, pas une certitude.",
  });
