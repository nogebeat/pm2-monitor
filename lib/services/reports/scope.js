...
async function resolveProcessScope(filters = {}, user = null) {
  ...
  if (user) {
    candidates = candidates.filter(
      (c) => permissions.hasPermission(user, c.processName, "view") && permissions.hasServerAccess(user, c.serverKey),
    );
  }

  // Ordre déterministe (utile pour les tests et un affichage stable).
  candidates.sort((a, b) => a.processName.localeCompare(b.processName) || a.serverKey.localeCompare(b.serverKey));
  return candidates;
}
