...
class AutoHealingService {
  ...
  async _attemptRestart(processName, source, reason, settings, attempt, now) {
    ...
    const backoffSeconds =
      settings.backoffSeconds[Math.min(attempt - 1, settings.backoffSeconds.length - 1)] || 0;
  ...
  async unblock(processName, user) {
    ...
    const updated = await this.stateStore.reset(processName, {
      unblockedBy: user && user.id !== undefined ? user.id : null,
    });
