...
function matches(silence, alert, processOrg) {
  switch (silence.scopeType) {
    case "rule":
      return (
        alert.ruleId !== undefined && alert.ruleId !== null && String(alert.ruleId) === silence.scopeValue
      );
    case "process":
    ...
    case "group":
      return (
        !!processOrg && Array.isArray(processOrg.groups) && processOrg.groups.includes(silence.scopeValue)
      );
    default:
      return false;
  }
