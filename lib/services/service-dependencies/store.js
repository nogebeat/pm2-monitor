...
async function listByHealthCheckId(healthCheckId) {
  const rows = await db.all(
    "SELECT * FROM service_dependencies WHERE health_check_id = ? AND enabled = 1",
    [healthCheckId],
  );
  return rows.map(rowToDependency);
}
...
async function update(id, changes) {
  ...
  await db.run(
    ...
    [
      ...
      merged.healthCheckId === undefined || merged.healthCheckId === null ? null : Number(merged.healthCheckId),
