...
async function buildGraphSnapshot({ healthChecksStore, listProcessStatuses } = {}) {
  ...
  const needsProcessStatus = dependencies.some(
    (d) => d.enabled && d.type === "PROCESS" && !d.healthCheckId,
  );
 ...
 */
async function computeImpact(serviceName, { assumeDown = false, healthChecksStore, listProcessStatuses } = {}) {
  ...
  const enabledEdges = snapshot.edges.filter((e) => e.enabled).map((e) => ({ source: e.source, target: e.target }));
