...
async function buildSnapshot(deps) {
  ...
  const [processesRaw, alerts, resolvedAlerts, healthChecks, recentEvents, recentHealings] = await Promise.all([
    listProcesses(),
    alertStore ? alertStore.listActive() : Promise.resolve(null),
    alertStore ? alertStore.listHistory({ state: "resolved", limit: RECENT_TIMELINE_LIMIT }) : Promise.resolve(null),
    ...
    autoHealingAuditStore ? autoHealingAuditStore.list({ limit: RECENT_TIMELINE_LIMIT }) : Promise.resolve(null),
  ]);
