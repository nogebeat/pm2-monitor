...
function defaultHttpRequest({ url, method, timeoutMs }) {
  return new Promise((resolve) => {
    ...
    const req = lib.request(
      target,
      { method: method || "GET", timeout: timeoutMs },
      (res) => {
        const chunks = [];
        res.on("data", (c) => {
...
function defaultTcpConnect({ host, port, timeoutMs }) {
  return new Promise((resolve) => {
    ...
    socket.once("timeout", () => finish({ ok: false, error: "timeout", responseTimeMs: Date.now() - started }));
    socket.once("error", (e) => finish({ ok: false, error: e.message, responseTimeMs: Date.now() - started }));
...
async function runCommand(check, { execFileImpl = defaultExecFile } = {}) {
  ...
  const expected = check.expectedExitCode === undefined || check.expectedExitCode === null ? 0 : check.expectedExitCode;
