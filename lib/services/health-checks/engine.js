...
class HealthCheckEngine {
  constructor({ store, ruleStore, alertEngine, now, onAlertResult, onCheckResult } = {}) {
    ...
    // Callback optionnel appelé après chaque exécution de run() avec le check
    // mis à jour (statut/compteurs) — utilisé par server.js pour diffuser
    // "health.updated" en websocket (Phase 8), sans coupler ce module à
    // Socket.IO.
    this.onCheckResult = typeof onCheckResult === "function" ? onCheckResult : null;
  }
  ...
  async run(checkId, impls) {
    ...
    if (this.onCheckResult) this.onCheckResult(updated);
    return updated;
  }
