...
function rowToCheck(row) {
  ...
  return {
    ...
    expectedExitCode: row.expected_exit_code === null || row.expected_exit_code === undefined
      ? 0
    ...
    lastCheckAt: row.last_check_at === null || row.last_check_at === undefined ? null : Number(row.last_check_at),
    lastResponseTimeMs:
...
function validate(input, { partial = false } = {}) {
  ...
  if (type === "command") {
    ...
    if (has("expectedExitCode")) {
      if (!Number.isInteger(Number(input.expectedExitCode))) errors.push("expectedExitCode doit être un entier.");
    }
  }
...
async function create(input, { userId } = {}) {
  ...
  const result = await db.run(
    `INSERT INTO health_checks
      (name, type, enabled, url, method, expected_status, expected_content, host, port,
...
async function update(id, changes) {
  ...
  await db.run(
    `UPDATE health_checks SET
      name = ?, type = ?, enabled = ?, url = ?, method = ?, expected_status = ?, expected_content = ?,
