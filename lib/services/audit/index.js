...
const ACTIONS = {
  ...
  // Incident Management & Alert Silencing (Phase 14) — une seule action pour
  // les transitions d'état (acknowledge/investigate/mitigate/resolve), le
  // détail (état de départ/d'arrivée) part en `metadata`, même raisonnement
  // que HEALTH_CHECK_CHANGE/PROCESS_ORG_CHANGE. Les silences ont leurs
  // propres actions (create/delete) : ce sont des objets à part entière
  // (table `alert_silences`), pas une transition d'incident.
  INCIDENT_STATE_CHANGE: "incident.state_change",
  INCIDENT_SILENCE_CREATE: "incident.silence_create",
  INCIDENT_SILENCE_DELETE: "incident.silence_delete",

  // Détection d'anomalies (Phase 16) — CRUD des règles, une seule action
  // (comme HEALTH_CHECK_CHANGE/PROCESS_ORG_CHANGE) : le détail
  // (create/update/delete) part en `metadata`.
  ANOMALY_RULE_CHANGE: "anomaly.rule_change",

  // Service Dependency Map (Phase 17) — une seule action pour le CRUD d'une
  // dépendance (create/update/delete/enable/disable), même raisonnement que
  // HEALTH_CHECK_CHANGE/ANOMALY_RULE_CHANGE : le détail part en `metadata`.
  DEPENDENCY_CHANGE: "dependency.change",

  // Advanced RBAC & API Keys (Phase 18, lib/services/api-keys/,
  // lib/routes/api-keys.js) — create/update/revoke sont des actions de
  // gestion classiques (comme HEALTH_CHECK_CHANGE), toujours auditées.
  // API_KEY_SENSITIVE_USE est distincte : elle trace l'UTILISATION d'une
  // clé API pour un scope marqué "dangereux" (voir
  // lib/permissions.js#SENSITIVE_API_KEY_SCOPES), pas sa gestion — voir
  // lib/auth.js#requirePermission et lib/process-helpers.js#withAppPermission.
  API_KEY_CREATE: "api_key.create",
  API_KEY_UPDATE: "api_key.update",
  API_KEY_REVOKE: "api_key.revoke",
  API_KEY_SENSITIVE_USE: "api_key.sensitive_use",

  // Backup & Restore (Phase 19, lib/services/backup/, lib/routes/backup.js)
  // — un export peut contenir des secrets déchiffrés (même si re-chiffrés
  // pour le fichier, voir docs/backup-restore/README.md) et une
  // restauration peut recréer des comptes/permissions : les deux sont
  // toujours auditées, succès ET échec.
  BACKUP_EXPORT: "backup.export",
  BACKUP_RESTORE: "backup.restore",

  // Plugin System (Phase 21, lib/services/plugins/, lib/routes/plugins.js)
  // — une seule action pour activer/désactiver/modifier la config d'un
  // plugin (comme HEALTH_CHECK_CHANGE/ANOMALY_RULE_CHANGE/DEPENDENCY_CHANGE) :
  // le détail (op: "enable"/"disable"/"config") part en `metadata`. Un
  // plugin exécutant du code arbitraire côté serveur, toute action de
  // gestion reste tracée, succès ET échec (même raisonnement que
